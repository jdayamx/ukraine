package jday.ukraine.item.custom;
...
public class UkraineArmorItem extends ArmorItem {
    private static final Map<ArmorMaterial, StatusEffect> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, StatusEffect>())
                    .put(UkraineArmorMaterials.FLAX_FABRIC, StatusEffects.INSTANT_HEALTH)
                    .build();

    public UkraineArmorItem(ArmorMaterial material, Type type, Settings settings) {
    ...
    private void addStatusEffectForMaterial(PlayerEntity player, ArmorMaterial mapArmorMaterial, StatusEffect mapStatusEffect) {
        ...
        if(hasCorrectArmorOn(mapArmorMaterial, player) && !hasPlayerEffect) {
            player.addStatusEffect(new StatusEffectInstance(mapStatusEffect, 1));

            if(new Random().nextFloat() > 0.6f) { // 40% of damaging the armor! Possibly!
                //player.getInventory().damageArmor(new DamageSource().inFire(), 1f, new int[]{0, 1, 2, 3});
            }
        }
    }
